...
object ScalaDataStructure extends App {
  // Data structures
  ...
  //It will reverse the every item string in reverse string format.
  val backwardShips = shipList.map((ship: String) => {
    ship.reverse
  })
  print("backwardShips : "+backwardShips)
  ...
  val sum = numberList.reduce((x: Int, y: Int) => x+y) // Check Why Tirnary operator not working.
  val max = numberList.reduce((x: Int, y: Int) => if(x > y)  x  else y) // Check Why Tirnary operator not working.
  //> sum  : Int = 15
  println("Maximum :"+max) //> 15
